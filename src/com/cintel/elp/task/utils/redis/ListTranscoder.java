package com.cintel.elp.task.utils.redis;
...
public class ListTranscoder<M extends Serializable> extends SerializeTranscoder {
	...
	public List<M> deserialize(byte[] in) {
	    ...
	    System.out.println(String.format("Caught IOException decoding %d bytes of data",  
			    in == null ? 0 : in.length) + e);
	} catch (ClassNotFoundException e) {  
	    System.out.println(String.format("Caught CNFE decoding %d bytes of data",  
			    in == null ? 0 : in.length) + e); 
	}  finally {
			close(is);
			close(bis);
	...
	public byte[] serialize(Object value) {
		...
		try {
			...
			System.out.println("serializable excepiton");
